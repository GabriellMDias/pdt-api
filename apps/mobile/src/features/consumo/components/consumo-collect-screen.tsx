...
import {
  createLocalConsumoEntry,
  getConsumoCollectedBalanceByProductAndReason,
  getLocalConsumoCatalogProductById,
  getLocalConsumptionReasonById,
  listLocalConsumptionReasons,
  searchLocalConsumoCatalog,
...
function resolveExactMatch(
  query: string,
  products: LocalConsumoCatalogProduct[],
): LocalConsumoCatalogProduct | null {
...
export function ConsumoCollectScreen() {
  ...
  const [availableReasons, setAvailableReasons] = useState<LocalConsumptionReason[]>([]);
  const [selectedReasonId, setSelectedReasonId] = useState<number | null>(parsedReasonId);
  const [selectedReason, setSelectedReason] = useState<LocalConsumptionReason | null>(null);
  const [selectedProduct, setSelectedProduct] = useState<LocalConsumoCatalogProduct | null>(null);
  const [suggestions, setSuggestions] = useState<LocalConsumoCatalogProduct[]>([]);
  ...
  const [movementType, setMovementType] = useState<ConsumoMovementType>('add');
  ...
  const scanContextKey =
    storeId != null && selectedReasonId != null
      ? buildConsumoScanContextKey(storeId, selectedReasonId)
      : '';
  ...
  const openScanner = useCallback(() => {
    if (!storeId || !selectedReasonId) {
      setNoticeTone('warning');
      setNoticeMessage('Selecione a loja e o tipo de consumo antes de abrir o leitor.');
  ...
  useEffect(() => {
    ...
    async function loadReason() {
      const [reasons, reason] = await Promise.all([
        listLocalConsumptionReasons(),
        selectedReasonId
          ? getLocalConsumptionReasonById(selectedReasonId)
  ...
  useEffect(() => {
    ...
    async function loadSuggestions() {
      ...
      const nextSuggestions = await searchLocalConsumoCatalog({
        storeId,
        query: deferredQuery,
  ...
  useEffect(() => {
    ...
    async function loadCollectedBalance() {
      ...
      const balance = await getConsumoCollectedBalanceByProductAndReason({
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function applyScannerSelection() {
        const product = await getLocalConsumoCatalogProductById(
          resolvedStoreId,
          selection.productId,
        );
    ...
    }, [consumeLookupFailure, consumeSelection, scanContextKey, selectedReasonId, storeId]),
  );
  ...
  async function handleSave() {
    ...
    if (quantityValue <= 0 || packageCountValue <= 0 || totalValue <= 0) {
      setNoticeTone('warning');
      setNoticeMessage('Informe quantidade e embalagem validas para salvar o consumo.');
      await playOperationalErrorAsync();
      return;
    }
    ...
    try {
      ...
      let nextMessage = `Lancamento de consumo salvo para o produto ${selectedProduct.id}.`;
      ...
      setNoticeMessage(
        error instanceof Error
          ? error.message
          : 'Nao foi possivel salvar o lancamento de consumo.',
      );
      await playOperationalErrorAsync();
    } finally {
      setIsSaving(false);
  ...
  if (!currentUser || !storeId || !selectedReasonId || !selectedReason) {
    return (
      <FeatureScreenLayout
        ...
        title="Consumo"
      >
        <Text style={styles.noticeText}>Nao foi possivel resolver a coleta de consumo.</Text>
      </FeatureScreenLayout>
    );
  }
  ...
  return (
    ...
    >
          ...
          <View style={styles.reasonBlock}>
            ...
            >
              <View style={styles.reasonSelectContent}>
                <Text style={styles.reasonValue}>{selectedReason.description}</Text>
              </View>
              <FontAwesome color={theme.colors.text.muted} name="chevron-down" size={16} />
            </Pressable>
          </View>
      ...
      <Card style={styles.formCard} variant="muted">
        ...
        <MovementTypeToggle value={movementType} onChange={setMovementType} />
      </Card>

      <View style={styles.productInfoWrap}>
        <Text style={styles.productName}>
          {selectedProduct?.description ?? 'Selecione um produto para salvar o lancamento.'}
        </Text>
        ...
        <View style={styles.lookupModalOverlay}>
          <View style={styles.lookupModalCard}>
            ...
            {scannerLookupModalCode ? (
              <Text style={styles.lookupModalCode}>Codigo lido: {scannerLookupModalCode}</Text>
            ) : null}
      ...
      <ConsumoReasonModal
        ...
        onConfirm={() => {
          if (selectedReasonId == null) {
            setNoticeTone('warning');
            setNoticeMessage('Selecione um tipo de consumo antes de continuar.');
            return;
          }
...
const createStyles = (theme: AppTheme) =>
  StyleSheet.create({
    ...
    productName: {
      backgroundColor: theme.isDark ? '#121212' : theme.colors.background.surfaceAlt,
      color: theme.colors.text.primary,
      fontSize: 20,
      lineHeight: 26,
