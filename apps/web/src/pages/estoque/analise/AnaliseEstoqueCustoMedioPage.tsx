...
export default function AnaliseEstoqueCustoMedioPage() {
  ...
  function formatDecimal(n: number | null | undefined) {
    if (n == null) return '';
    return n.toLocaleString('pt-BR', {
  ...
  async function consultar(opts?: {
    ...
    try {
      ...
      const url = `${API_BASE}/api/analysis/stock/diferenca-custo-medioxultimo/diario?${qs.toString()}`;
      ...
      if (!resp.ok) {
        ...
        if (!ct.includes('application/json')) {
          throw new Error(
            `Erro HTTP ${resp.status}. Resposta nao e JSON.\n${text.slice(0, 200)}`,
          );
        }
      ...
      const data: ResumoDia[] = await resp.json();
      setRows(data);
    } catch (e: any) {
  ...
  function abrirDetalheDia(row: ResumoDia) {
    const dateOnly =
    ...
    navigate(`/estoque/analises/custo-medioxultimo/dia?${qs.toString()}`);
  }
  ...
  function formatDatePTBR(input: string): string {
    ...
    return Number.isNaN(d.getTime()) ? input : d.toLocaleDateString('pt-BR');
  }
  ...
  const columns: Column<ResumoDia>[] = [
    ...
    },
    {
      ...
      align: 'right',
      sortable: true,
      sortAccessor: (r) => r.qtd_registros,
      resizable: true,
      cell: (r) => formatInt(r.qtd_registros),
    },
    {
      key: 'diferenca_total',
      header: 'Diferenca total',
      align: 'right',
      sortable: true,
      sortAccessor: (r) => r.diferenca_total,
      resizable: true,
      cell: (r) => (
        <span className={r.diferenca_total >= 0 ? 'text-emerald-600' : 'text-red-600'}>
          {formatDecimal(r.diferenca_total)}
        </span>
      ),
    },
  ];
  ...
  return (
    <Layout title="Analise de Estoque - Custo Medio x Ultimo">
      <FullscreenLoader open={openingDetail} label="Abrindo detalhamento..." />
      <div className="rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white/95 dark:bg-pilar-default-bg2-dark p-5 shadow-sm text-neutral-800 dark:text-neutral-100">
        ...
        <div className="mt-4">
          ...
          <p className="mt-3 text-center text-xs text-neutral-500 dark:text-neutral-400">
            De duplo clique em uma linha para abrir o detalhamento por dia.
          </p>
        </div>
      </div>
    </Layout>
