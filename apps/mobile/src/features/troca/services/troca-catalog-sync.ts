...
export async function syncExchangeReasonsCatalog(payload: {
  userId: number;
  storeId: number;
  ...
  try {
    ...
    if (response.domain !== 'exchange.reasons') {
    ...
    const persistenceMetric = await measureSyncPhase('persist.catalog_items', () =>
      replaceExchangeReasons(normalizationMetric.result),
    );
    const metrics = [
      {
    ...
    emitSyncMetricsLog('exchange.catalog.reasons', {
      storeId: payload.storeId,
      itemsCount: response.items.length,
      totalDurationMs,
      metrics,
    ...
    await finishSyncRun(runId, {
      status: 'failed',
      finishedAt: new Date().toISOString(),
      errorCode: 'exchange_reasons_sync_failed',
      errorMessage:
        error instanceof Error ? error.message : 'Falha ao sincronizar os motivos de troca.',
    });
