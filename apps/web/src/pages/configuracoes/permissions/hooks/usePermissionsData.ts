  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      for (const code of changedCodes) {
        ...
        // Diferenças por loja:
        // - se NÃO global, aplica diff normal
        // - se global, remove qualquer vínculo por loja anterior (evita lixo no banco)
        if (isStore) {
          ...
          if (!curr.global) {
            ...
            for (const st of prevSet) addTo(perStoreDisable, st, code);
          }
        }
      }
