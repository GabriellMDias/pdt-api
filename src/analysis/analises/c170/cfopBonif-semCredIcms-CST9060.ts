...
export const CfopBonifSemCredICMSCST9060 = {
  code: 'CFOPBonif_semCredICMSCST9060',
  description: 'Entrada de Bonificacao sem Credito - CST = x90 ou x60',
  groupName: 'C170',
  fields: [
    ...
    for (const [chave, nota] of notas.entries()) {
      for (const item of nota.itens) {
        ...
        if (item.CFOP === '1910' || item.CFOP === '2910') {
          if (item.VL_ICMS === 0) {
            if (item.CST_ICMS !== '090' && item.CST_ICMS !== '060') {
                erros.push({
                chave,
                ...
                erro: `Entrada de Bonificacao sem Credito de ICMS com CST Diferente de x90 ou x60`,
              });
            }
          }
        }
