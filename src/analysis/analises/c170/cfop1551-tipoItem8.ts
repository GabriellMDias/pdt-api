...
export const Cfop1551TipoItem8 = {
  code: 'CFOP1551_TIPOITEM8',
  description: 'Ativo Imobilizado - CFOP 1.551 x Tipo Item Diferente de 08',
  groupName: 'C170',
  fields: [
  ...
  execute(
    ...
    for (const [chave, nota] of notas.entries()) {
      for (const item of nota.itens) {
        // Regra: se CFOP = 1551 → TIPO_ITEM deve ser 8
        if (item.CFOP === '1551') {
          ...
          if (cad.TIPO_ITEM !== '08') {
            erros.push({
              ...
              erro: `CFOP 1551 exige Tipo Item = 08, mas encontrado ${cad.TIPO_ITEM}`,
            });
          }
        }
      }
