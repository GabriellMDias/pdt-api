// src/pages/configuracoes/db-scripts/helpers.ts
import type { DbScript } from "./types";

const WEEKDAYS = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"];

export function scheduleToText(s: DbScript): string {
  switch (s.scheduleType) {
    ...
    case "DAILY_AT":
      return `Diário às ${s.dailyTime}${s.timezone ? ` @ ${s.timezone}` : ""}`;
    case "WEEKLY_AT":
